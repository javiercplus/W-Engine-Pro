    ...
    def _detect_gpu(self):
        try:
            if shutil.which("lspci"):
                output = subprocess.check_output("lspci | grep -i vga", shell=True).decode().lower()
                if "nvidia" in output:
                    return "nvidia"
            ...
            if shutil.which("glxinfo"):
                output = subprocess.check_output("glxinfo | grep 'OpenGL vendor'", shell=True).decode().lower()
                if "nvidia" in output:
                    return "nvidia"
