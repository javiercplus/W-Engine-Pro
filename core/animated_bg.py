...
class BackgroundManager:
    ...
    def get_window_info(self):
        ...
        try:
            win_id = self.root.get_full_property(self.ACTIVE_WINDOW, X.AnyPropertyType).value[0]
            if win_id == 0: return True, False
    ...
    def manage_bg(self, is_desktop, is_fs):
        if is_fs:
            if self.state != "STOPPED":
                print("[GAME MODE] Pantalla completa detectada. Matando proceso para liberar recursos.")
                self.stop()
        elif is_desktop:
            if self.state == "STOPPED":
                self.start()
    ...
    def start(self):
        self.stop()
        res = subprocess.check_output("xdpyinfo | grep dimensions | awk '{print $2}'", shell=True).decode().strip()
        cmd = [
            "xwinwrap", "-ov", "-b", "-ni", "-fs", "-nf", "--", 
            "mpv", "--wid=%WID", "--loop", "--no-audio", "--idle",
            "--vo=gpu", "--hwdec=auto", f"--input-ipc-server={SOCKET_PATH}", 
            VIDEO_PATH
        ]
