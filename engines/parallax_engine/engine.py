...
from core.compositor_detector import CompositorDetector
import logging
import json
import os
...
class ParallaxEngine(WallpaperEngineInterface):
    ...
    def init(self, surface_handle, monitor_info):
        ...
        self.view.setWindowFlags(Qt.FramelessWindowHint | Qt.WindowStaysOnBottomHint | Qt.Tool)
    ...
    def set_wallpaper(self, config_path):
        ...
        try:
            if os.path.isdir(config_path):
                files = sorted([os.path.join(config_path, f) for f in os.listdir(config_path) if f.endswith('.png')])
